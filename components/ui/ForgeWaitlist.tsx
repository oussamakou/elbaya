...
export default function ForgeWaitlist({button}: {button: string}) {
  ...
  return (
    <form action={submit} className="mx-auto mt-8 flex max-w-xl flex-col gap-3 sm:flex-row">
      ...
      <button className="rounded-full bg-terracotta px-6 py-3 text-sm font-medium text-cream transition hover:bg-[#A8501C] active:scale-[0.97]">{done ? t('notify') : button}</button>
    </form>
  );
}
