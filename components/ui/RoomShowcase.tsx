    ...
    <section className="px-5 pb-24">
      <div className="mx-auto grid max-w-7xl items-stretch gap-6 md:grid-cols-2">
        <div className="grid gap-4">
          <div className="relative aspect-[4/3] overflow-hidden rounded-[8px] bg-mist ring-1 ring-inset ring-earth/10">
          ...
          <div className="relative aspect-[16/9] overflow-hidden rounded-[8px] bg-mist ring-1 ring-inset ring-earth/10">
        ...
        <div className="flex flex-col justify-center rounded-[8px] border border-mist bg-cream p-7 md:p-10">
          <h2 className="font-serif text-5xl italic">{t.name}</h2>
          <p className="mt-5 leading-8 text-earth/75">{t.intro}</p>
          <ul className="mt-7 space-y-2 text-sm leading-6 text-earth/80">
            {t.specs.map((s) => <li key={s}>· {s}</li>)}
          </ul>
          <div className="mt-6 border-t border-mist pt-5">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-bark">{locale === 'fr' ? 'En option' : 'Add-ons'}</p>
            <ul className="mt-3 space-y-2 text-sm leading-6 text-earth/70">
