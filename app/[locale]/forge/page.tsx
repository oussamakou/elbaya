    ...
    <div className="grain min-h-screen bg-dusk text-cream">
      <section className="relative flex min-h-screen items-center px-5 pt-20">
        <div className="relative mx-auto max-w-6xl">
          <p className="text-xs font-medium uppercase tracking-label text-olive">{content.hero.label}</p>
          <h1 className="mt-8 whitespace-pre-line font-serif text-6xl italic leading-[0.96] md:text-[80px]">{content.hero.heading}</h1>
          <p className="mt-8 max-w-xl text-lg leading-8 text-cream/70">{content.hero.body}</p>
          <div className="mt-9 flex flex-wrap gap-4 text-sm uppercase tracking-label text-cream/70">{content.stats.map((stat) => <span key={stat}>{stat}</span>)}</div>
      ...
      <section className="relative px-5 py-24">
        <div className="mx-auto max-w-5xl divide-y divide-cream/12">
          {content.week.map((item) => <div key={item[0]} className="grid gap-5 py-9 md:grid-cols-[.45fr_1fr]"><h2 className="text-sm font-medium tracking-label text-cream">{item[0]}</h2><p className="max-w-xl text-xl leading-8 text-cream/70">{item[1]}</p></div>)}
        </div>
      </section>
      ...
      <section className="relative px-5 py-16">
        <div className="mx-auto grid max-w-5xl gap-4 md:grid-cols-4">{content.included.map((item) => <div key={item} className="border border-cream/12 p-5 text-sm uppercase tracking-label text-cream/70">{item}</div>)}</div>
      </section>
      <section className="relative px-5 py-20">
        <div className="mx-auto max-w-4xl space-y-5">{content.faq.map((item) => <details key={item[0]} className="border-b border-cream/12 py-5"><summary className="cursor-pointer font-serif text-3xl italic">{item[0]}</summary><p className="mt-4 leading-8 text-cream/70">{item[1]}</p></details>)}</div>
      </section>
      <section id="apply" className="relative px-5 py-28 text-center">
        ...
        <p className="mt-5 text-sm text-cream/70">{locale === 'fr' ? 'Réponse sous 48 heures.' : 'You will hear back within 48 hours.'}</p>
      </section>
